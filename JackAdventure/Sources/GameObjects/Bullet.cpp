...
void Bullet::Init()
{
	...
	m_runAni = new Animation(*DATA->getTexture("Boss1/poison-spit"), sf::Vector2i(4, 3), 0.1f);
	m_deathAni = new Animation(*DATA->getTexture("Boss1/poison-impact"), sf::Vector2i(4, 3), 0.1f);
...
void Bullet::Update(float deltaTime)
{
	...
	else
	{
		...
		Animation* ani = (Animation*)m_currentAni;
		if (ani->getCurrentFrame() == ani->getFrameNum()) {
			m_currentAni->Update(deltaTime);
			m_currentAni->setPosition(getHitBox()->getPosition());
		}
		else m_stop = true;
